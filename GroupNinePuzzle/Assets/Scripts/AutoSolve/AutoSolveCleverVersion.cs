...
public class AutoSolveCleverVersion : MonoBehaviour
{
    ...
    List<GameObject>[,] potentialPieces = new List<GameObject>[100, 100];
    ...
    public void AutoSolve()
    {
        ...
        currentColumn = 0;
        theta = 90.0f;
        indexOfTheta = 0;     //starting theta is always 90 degrees
        int numberOfPieces = pieces.Count;

        FindPotentialPieces();
        if (potentialPieces[currentRow, currentColumn].Count == 0)
        {
        ...
        // PlacePiece();
        var columnInPreviousLoop = 0;
        var rowInPreviousLoop = 0;

        var preventInfiniteLoop = 0;
        while (placedPieces.Count < numberOfPieces)
        {
            Debug.Log("counter: " + preventInfiniteLoop);
            preventInfiniteLoop++;
            if (preventInfiniteLoop > 3000)
            {
                Debug.Log("Stopped at 3000");
                break;
            }
            Debug.Log("*********************************************************");
            ...
            if (changedRows) FindPotentialPieces();
            Debug.Log("number of potential pieces: " + potentialPieces[currentRow, currentColumn].Count);
            // if (potentialPieces[currentRow, currentColumn].Count == 0)
            // {
            ...
            SetActivePiece();
            Debug.Log("row and column: (" + currentRow + ", " + currentColumn + ")");
            if (potentialPieces[currentRow, currentColumn].Count == 0 && currentRow == 0 && currentColumn == 0)
            {
                Debug.Log("nothing left to try");
                break;
            }
            PlacePiece();
            ...
            while (overlap == true)
            {
                ...
                if (preventInfiniteLoop2 > 50)
                {
                    Debug.Log("Stopped at 50");
                    break;
                }
                ...
                {
                    Debug.Log("piece name when overlap checking: " + activePiece.name);
                    Debug.Log("number of theta angles: " + activePiece.GetComponent<PieceInfo>().thetaAngles.Count);
                ...
                else if (activePiece.GetComponent<PieceInfo>().thetaAngles.Count == 1)
                {
                    ...
                    var resultOfRemoval = potentialPieces[currentRow, currentColumn].Remove(activePiece);
                    Debug.Log("Successful removal of potential piece: " + resultOfRemoval);
                    overlap = OverLapsBoard();
                    continue;
                }
                else
                {
                    potentialPieces[currentRow, currentColumn].Remove(activePiece);
                    Debug.Log("In else statement. Number of potential: " + potentialPieces[currentRow, currentColumn].Count);
    ...
    void SetActivePiece()
    {
        Debug.Log("SetActive currentRow: " + currentRow);
        Debug.Log("SetActive currentColumn: " + currentColumn);
        ...
        activePiece = potentialPieces[currentRow, currentColumn][0];

        if (activePiece.GetComponent<PieceInfo>().thetaAngles.Count == 0)
        {
            Debug.Log("No theta angles in piece: " + activePiece.name);
            potentialPieces[currentRow, currentColumn].Remove(activePiece);
            SetActivePiece();
            return;
        }
    ...
    void Backtrack()
    {   //we reach this point when potentialPieces is empty and nothing fits
        ...
        else
        {
            ...
            var startingRow = currentColumn == 99 ? currentRow + 1 : currentRow;
            var startingColumn = currentColumn == 99 ? 0 : currentColumn + 1;
            EmptyPotentialPiecesFromRowColumn(startingRow, startingColumn);

            foreach (var item in placedPieces)
            {
                Debug.Log("placedPiece: " + item.piece.name);
                Debug.Log("placedPiece row column: " + item.row + " " + item.column);

            }

            var result = placedPieces.Remove(pieceToRemove);
            Debug.Log("removal successful: " + result);
            ...
            Debug.Log("name of active piece: " + activePiece.name);

            // FindPotentialPieces();
            Debug.Log("upon backtracking " + potentialPieces[currentRow, currentColumn].Count + " potential pieces were found");
            foreach (var item in potentialPieces[currentRow, currentColumn])
            {
            ...
            // {
            SetActivePiece();
            if (potentialPieces[currentRow, currentColumn].Count == 0) return; // no pieces left
            // PlacePiece();
            // }
            // if (potentialPieces[currentRow, currentColumn].Count == 0)
            // {
    ...
    void CalculateNextAngle()
    {
        if (currentColumn > 0)
        {
            ...
            int n = 0;
            while (n < vertices.Length && currentPoint != vertices[n])
            {
                n++;
            }
            Vector3 nextVertex;
    ...
    void FindPotentialPieces()
    {
        ...
        var potentialPiecesForRowColumn = new List<GameObject>();       //reset potPieces
        foreach (GameObject testPiece in pieces)
        {
            ...
            foreach (float angle in angles)
            {
                if (angle <= theta + 0.01 && angle >= theta - 0.01)
                {
                    ...
                    potentialPiecesForRowColumn.Add(testPiece);
                    break;
                }
            }
        }
        potentialPieces[currentRow, currentColumn] = potentialPiecesForRowColumn;
    ...
    void EmptyPotentialPiecesFromRowColumn(int startingRow, int startingColumn)
    {
        for (int i = startingRow; i < 100; i++) // 100 is length of potential pieces in each dimension
        {
            for (int j = startingColumn; i < 100; i++)
            {
                potentialPieces[i, j] = new List<GameObject>();
            }
            startingColumn = 0;
        }
    }
